...
const OPENAI_FETCH_TIMEOUT_MS = 45_000; // stay under Vercel's 60s function ceiling
const PDF_PAGE_CONCURRENCY = 4;

// fetch() has no built-in timeout — if OpenAI's API stalls, a plain fetch hangs forever,
// which can leave the client-side spinner stuck even after Vercel kills the function.
// This always rejects on its own clock so callers get a real error to catch and display.
export async function fetchWithTimeout(url, options, timeoutMs = OPENAI_FETCH_TIMEOUT_MS) {
...
function visionModelCandidates() {
  return unique([process.env.OPENAI_VISION_MODEL, "gpt-4.1-mini", "gpt-4.1", "gpt-4o-mini", "gpt-4o"]);
}
...
function betterString(current, next) {
  const a = String(current || "").trim();
  const b = String(next || "").trim();
  return b.length > a.length ? b : a;
}

function mergePriceTables(current, next) {
  if (!next?.columns?.length && !next?.rows?.length) return current || null;
  if (!current?.columns?.length && !current?.rows?.length) return next;

  const columns = current?.columns?.length >= next?.columns?.length ? current.columns : next.columns;
  const rows = [];
  const seen = new Set();
  for (const row of [...(current?.rows || []), ...(next?.rows || [])]) {
    const key = `${row?.dates || ""}|${(row?.cells || []).join("|")}`;
  ...
  return {
    columns: columns || [],
    rows,
    note: betterString(current?.note, next?.note),
  };
}

function mergeExtractedTrips(parts) {
  const merged = normalizeExtractedTrip({
    title: "",
    subtitle: "",
    duration_days: 0,
    duration_nights: 0,
  ...
  for (const part of parts) {
    if (!part) continue;
    merged.title = betterString(merged.title, part.title);
    merged.subtitle = betterString(merged.subtitle, part.subtitle);
    merged.duration_days = Math.max(Number(merged.duration_days || 0), Number(part.duration_days || 0));
    merged.duration_nights = Math.max(Number(merged.duration_nights || 0), Number(part.duration_nights || 0));
    if (!merged.flights && part.flights) merged.flights = part.flights;
    merged.price_note = betterString(merged.price_note, part.price_note);
    merged.price_table = mergePriceTables(merged.price_table, part.price_table);

    for (const departure of part.departures || []) {
      const date = String(departure?.date || "").trim();
      if (!date || departureDates.has(date)) continue;
      departureDates.add(date);
      merged.departures.push({ date });
    }

    for (const value of part.includes || []) {
      const text = String(value || "").trim();
      if (text) includeSet.add(text);
    }
    for (const value of part.excludes || []) {
      const text = String(value || "").trim();
      if (text) excludeSet.add(text);
    }
    ...
    for (const day of part.days || []) {
      ...
      if (!existing || JSON.stringify(day).length > JSON.stringify(existing).length) {
        dayMap.set(dayNumber, day);
      }
    }
  }

  merged.days = [...dayMap.values()].sort((a, b) => Number(a.day || 0) - Number(b.day || 0));
  merged.includes = [...includeSet];
  merged.excludes = [...excludeSet];
  return normalizeExtractedTrip(merged);
}
...
async function splitPdfPages(buffer, filename) {
...
export async function extractTripFromPdfPages(buffer, filename = "document.pdf") {
  const pages = await splitPdfPages(buffer, filename);
  if (pages.length <= 1) return extractTripFromPdf(pages[0]?.base64 || buffer.toString("base64"), filename);

  console.log(`[openai] extracting ${pages.length} PDF pages with concurrency ${PDF_PAGE_CONCURRENCY}`);
  const settled = await mapWithConcurrency(
    pages,
    PDF_PAGE_CONCURRENCY,
    (page) => extractTripFromPdf(page.base64, page.filename)
  );
  ...
  for (const [index, result] of settled.entries()) {
    if (result.status === "fulfilled") {
      ...
      console.warn(`[openai] page ${index + 1} extraction failed:`, result.reason?.message || result.reason);
    }
  }

  if (trips.length === 0) {
    throw new Error("OpenAI could not extract any PDF pages.");
  }

  return mergeExtractedTrips(trips);
}
