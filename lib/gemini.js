...
// Plain-text documents (docx/txt) have no visual layout to read — text-specific rules.
const TEXT_USER_PROMPT = `Create structured data for a UUDAM Travel poster from the document text below.
...
- Do NOT leave cells empty. Each row's cells array must match columns length exactly with the actual price values.
- If a price has both yuan and tugrik, put both in the cell like "4,180 юань / 2,340,000₮".
- The dates field per row should be the departure date(s), copied as written in the source.
...
// Shared Gemini call: parts = user-message content parts (inline_data / text).
...
export async function extractTripFromImageGemini(base64, mimeType = "image/jpeg") {
  return geminiExtract([
...
export async function extractTripFromTextGemini(docText) {
  return geminiExtract([{ text: TEXT_USER_PROMPT + docText }]);
}
