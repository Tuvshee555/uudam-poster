  ...
  try {
    ...
    for (const page of pdfDoc.getPages()) {
      ...
      for (const key of xObjects.keys()) {
        ...
        console.log(`[pdfImages] image key=${key} size=${bytes.length} filter=${filterStr}`);
      ...
      if (bestBytes) {
        ...
        console.log(`[pdfImages] page image found: ${bestBytes.length} bytes`);
      } else {
        console.log(`[pdfImages] page: no JPEG image found`);
      }
    }

    console.log(`[pdfImages] total images extracted: ${images.length}`);
    return images;
  } catch (e) {
